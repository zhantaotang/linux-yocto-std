...
static u32 s32gen1_pcie_get_soc_revision(void)
{
	...
	if (siul2_base_address != OF_BAD_ADDR) {
		void __iomem *siul2_virt_addr = ioremap(
...
#ifdef DEBUG_R
	if (base == pci->dbi_base)
		pr_debug_r("%s(pcie%d): R%lu(dbi+0x%x) => ", __func__,
				s32_pci->id, size * 8, (reg));
	else if (base == s32_pci->atu_base)
		pr_debug_r("%s(pcie%d): R%lu(atu+0x%x) => ", __func__,
				s32_pci->id, size * 8, (reg));
	else if (base == pci->dbi_base2)
		pr_debug_r("%s(pcie%d): R%lu(dbi2+0x%x) => ", __func__,
				s32_pci->id, size * 8, (reg));
	else if (base == s32_pci->ctrl_base)
		pr_debug_r("%s(pcie%d): R%lu(ctrl+0x%x) => ", __func__,
				s32_pci->id, size * 8, (reg));
#endif
...
int s32_pcie_setup_outbound(struct s32_outbound_region *ptrOutb)
{
	...
	ret = epc->ops->map_addr(epc, 0, 0, ptrOutb->base_addr,
...
int s32_pcie_setup_inbound(struct s32_inbound_region *inbStr)
{
	...
	ret = epc->ops->set_bar(epc, 0, 0, &s32gen1_ep_bars[bar_num]);
#else
	bar.barno = bar_num;
